...
# Simulated compass: Earth magnetic field for Chennai, India in NED frame (Gauss)
# N=0.280 E=0.020 D=0.360 — rotated to body frame each cycle for perfect HIL heading
    ...
    while is_running:
        try:
            ...
            if cmd in ['qloiter', 'loiter']:
                ...
                # Use QHOVER (altitude-only hold) for takeoff — does NOT need yaw for stabilization.
                # QLOITER needs yaw to rotate position corrections NED→body; wrong yaw = crash.
                # QHOVER only needs roll/pitch/alt — all reliable from Gazebo IMU.
                fc.set_mode(18)  # QHOVER: altitude hold only, no position, no yaw dependency
                fc.mav.command_long_send(
                    fc.target_system, fc.target_component,
                    mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM,
                    0, 1, 21196, 0, 0, 0, 0, 0
                )
                print(f"[CMD] >>> QHOVER TAKEOFF! Ascending directly to 4m... <<<")
                throttle_val = 1650  # Solid positive climb rate in QHOVER
                is_taking_off = True
                loiter_target_alt = 8.0
...
def main():
    ...
    hitl_overrides = {
        ...
        'EK3_SRC2_POSZ': 0.0,
        'EK3_SRC1_YAW': 2.0,
        'EK3_SRC2_YAW': 0.0, 'EK3_SRC3_YAW': 0.0,
        ...
        'SERVO5_MIN': 1000.0, 'SERVO5_MAX': 2000.0, 'SERVO5_TRIM': 1500.0, 'SERVO5_REVERSED': 1.0,
    ...
    # Latest sensor state — updated by Gazebo, injected into EKF at 400 Hz
    # (Sending only when Gazebo fires gives ~50 Hz → EKF attitude gaps → crashes)
    hil_accel  = [0.0, 0.0, 9.81]
    hil_gyro   = [0.0, 0.0, 0.0]
    hil_mag    = [MAG_NED[0], 0.0, MAG_NED[2]]  # level body = NED field
    hil_press  = 1013.25
    hil_alt    = HOME_ALT
    last_hil_time = 0.0
    HIL_DT = 1.0 / 400.0  # 2.5 ms — match EKF sample rate exactly
    ...
    while is_running:
        ...
        try:
            ...
            if is_taking_off and rel_alt >= 4.0:
                is_taking_off = False
                throttle_val = 1500
                try:
                    fc.set_mode(19)
                    ...
                    pass
                print(f"[CMD] >>> AT {rel_alt:.1f}m: Switched QHOVER -> QLOITER. Position hold ACTIVE! <<<")

            lat = HOME_LAT + (x_north * DEG_PER_METER)
            lon = HOME_LON + (y_east * DEG_PER_METER / math.cos(math.radians(HOME_LAT)))

            vn = float(vel[0]); ve = float(vel[1]); vd = -float(vel[2])
            ...
            yaw_cdeg = int((math.degrees(yaw) % 360) * 100) or 36000

            # Simulated compass: rotate NED field into body frame
            mn, me, md = MAG_NED
            mx = (1-2*(qy*qy+qz*qz))*mn + 2*(qx*qy+qw*qz)*me + 2*(qx*qz-qw*qy)*md
            my = 2*(qx*qy-qw*qz)*mn + (1-2*(qx*qx+qz*qz))*me + 2*(qy*qz+qw*qx)*md
            mz = 2*(qx*qz+qw*qy)*mn + 2*(qy*qz-qw*qx)*me + (1-2*(qx*qx+qy*qy))*md

            # Update latest HIL sensor state (injected at 400 Hz below)
            hil_accel = [float(accel_raw[0]), float(accel_raw[1]), float(accel_raw[2])]
            hil_gyro  = [float(gyro[0]), float(gyro[1]), float(gyro[2])]
            hil_mag   = [mx, my, mz]
            hil_press = float(1013.25 * math.pow(1.0 - (amsl_alt / 44330.0), 5.255))
            hil_alt   = float(amsl_alt)

            # Actuator passthrough: send PWM to Gazebo every physics frame
            frame_count += 1
            gz_pwm = list(last_pwm)
            raw_yaw = last_pwm[6] if abs(last_pwm[6]-1500) > abs(last_pwm[4]-1500) else last_pwm[4]
            # On ground (<0.15m), lock rear tilt servo to 1500 (0 deg) and equalize front motors:
            if rel_alt < 0.15:
                gz_pwm[4] = 1500
                max_front = max(last_pwm[0], last_pwm[1])
                gz_pwm[0] = max_front
                gz_pwm[1] = max_front
            else:
                gz_pwm[4] = raw_yaw
            ...
            # GCS attitude telemetry
            att = fc.mav.attitude_encode(boot_time_ms, float(roll), float(pitch), float(yaw),
        ...
        # ─── B. Stream HIL_SENSOR at constant 400 Hz (EKF rate) ───────────────────────────
        # CRITICAL: Send regardless of whether Gazebo gave us a new frame.
        # At ~50 Hz Gazebo frames, the EKF was starved for 7 cycles between frames → attitude jumps.
        # Now we feed the EKF the last-known accel/gyro/baro every 2.5 ms continuously.
        if now - last_hil_time >= HIL_DT:
            last_hil_time = now
            try:
                fc.mav.hil_sensor_send(
                    int(now * 1e6),
                    hil_accel[0], hil_accel[1], hil_accel[2],
                    hil_gyro[0],  hil_gyro[1],  hil_gyro[2],
                    hil_mag[0],   hil_mag[1],   hil_mag[2],
                    hil_press, 0.0, hil_alt, 20.0,
                    0x1FFF
                )
            except Exception:
                pass

        # ─── B. Send Clean 20 Hz Precision GPS to Matek & Mission Planner ───
        if now - last_gps_time > 0.05:
            last_gps_time = now
